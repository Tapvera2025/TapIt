...
import { installDevelopmentPrincipalResolver } from './context.js';
 ...
 *
 * It is retained only for explicit local pipeline tests. Normal application
 * startup uses Identity, including in development.
 */
...
export function installDevPrincipalResolver(options: { allowIdentityOverride: boolean }): void {
  if (process.env['NODE_ENV'] === 'production') {
    throw new Error(
      'installDevPrincipalResolver() must never run in production. It trusts a header.',
    );
  }
  if (!options.allowIdentityOverride) {
    throw new Error('Development bypass requires explicit IDENTITY_DEV_BYPASS=true');
  }

  installDevelopmentPrincipalResolver(async (req: Request) => {
