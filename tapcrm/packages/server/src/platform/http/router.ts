...
import { requireModuleEnabled } from '../module-entitlement.js';
...
export function assertManifest(options: { strict?: boolean } = {}): ManifestDrift {
  ...
  if (fatal.length > 0) {
    throw new Error(
      `Route manifest drift — refusing to start.\n\n  ${fatal.join('\n\n  ')}`,
    );
  }
...
export function buildRouter(): Router {
  ...
  for (const binding of registeredBindings()) {
    const method = binding.method.toLowerCase() as
  ...
  return async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      if (binding.module !== undefined) {
        await requireModuleEnabled(ctx, binding.module);
      }
