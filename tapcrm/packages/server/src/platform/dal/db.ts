...
export const bootstrapDb = {
  /** Read the global Identity email directory before a tenant is known. */
  async readIdentityDirectory<T>(fragment: SqlFragment): Promise<T[]> {
...
export const platformDb = {
  async one<T>(
    ...
    const rows = await this.query<T>(operation, reason, fragment);
    if (rows.length !== 1)
      throw new Error(`Expected exactly one row, got ${rows.length}`);
    return rows[0]!;
  },
  ...
  async transactionForOrganization<T>(
    ...
    if (!organizationId)
      throw new MissingTenantContextError('platform transaction has no organizationId');
    console.warn(
      JSON.stringify({
        level: 'warn',
        msg: 'platformDb tenant-scoped provisioning transaction',
        operation,
        reason,
        organizationId,
      }),
