...
export function SessionsPage({ onBack, onSignedOut }: { onBack: () => void; onSignedOut: () => void }): React.JSX.Element {
  ...
  return <main className="min-h-screen bg-app-background p-6 text-app-foreground md:p-10">
    <div className="mx-auto max-w-5xl">
      ...
      <div className="page-heading flex flex-wrap items-center justify-between gap-5">
        ...
        {!loading && sessions.length > 0 && <button type="button" onClick={() => setConfirm({ kind: 'all' })} className="rounded-lg border border-[#d86b6b]/40 px-4 py-2.5 text-sm font-bold text-app-danger hover:bg-[#d86b6b]/10">Sign out everywhere</button>}
      </div>
