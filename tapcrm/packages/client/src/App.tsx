...
import { ForgotPasswordPage, IdentityLoginPage, PasswordChangePage, ResetPasswordPage, clearIdentityTokens, getIdentityAccessToken, identityLogout } from './identity/index.js';
...
export function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(IDENTITY_EXPIRED_EVENT, handleIdentityExpired);
  }, []);
  ...
  function handleIdentityLogin(result: IdentityLoginResult) {
    ...
    navigate(result.passwordChangeRequired ? '/company/password-change' : '/company/dashboard');
  }
  ...
  if (pathname === '/login') {
    return companyAuthenticated ? (
      ...
      <IdentityLoginPage onSuccess={handleIdentityLogin} onForgotPassword={() => navigate('/forgot-password')} />
    );
  }
  if (pathname === '/forgot-password') return <ForgotPasswordPage onBack={openCompanyLogin} />;
  if (pathname === '/reset-password') {
    const params = new URLSearchParams(window.location.search);
    return <ResetPasswordPage token={params.get('token') ?? ''} organizationCode={params.get('org') ?? ''} onComplete={openCompanyLogin} />;
  }
  if (pathname === '/company/password-change') {
    if (!companyAuthenticated || !passwordChangeRequired) {
      window.history.replaceState({}, '', '/login');
      return <IdentityLoginPage onSuccess={handleIdentityLogin} onForgotPassword={() => navigate('/forgot-password')} />;
    }
    return <PasswordChangePage onComplete={() => {
      clearIdentityTokens();
      setCompanyAuthenticated(false);
      setPasswordChangeRequired(false);
      navigate('/login');
  ...
  if (pathname === '/company/dashboard') {
    ...
    return <CompanyWorkspace pathname={pathname} onNavigate={navigate} onLogout={() => {
